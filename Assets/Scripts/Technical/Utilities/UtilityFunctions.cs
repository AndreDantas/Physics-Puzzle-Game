...
public static class UtilityFunctions
{
    ...
    public static void GizmosDrawSquare(Vector2 pos, float size = 1f, Color? gizmosColor = null)
    {
        Gizmos.color = gizmosColor ?? Color.green;

        //Left
        Gizmos.DrawLine(new Vector3(pos.x - size / 2f, pos.y - size / 2f), new Vector3(pos.x - size / 2f, pos.y + size / 2f));
        //Right
        Gizmos.DrawLine(new Vector3(pos.x + size / 2f, pos.y - size / 2f), new Vector3(pos.x + size / 2f, pos.y + size / 2f));
        //Up
        Gizmos.DrawLine(new Vector3(pos.x - size / 2f, pos.y + size / 2f), new Vector3(pos.x + size / 2f, pos.y + size / 2f));
    ...
    public static void GizmosDrawGrid(Vector2 bottomLeftPivotPosition, int columns = 1, int rows = 1, float cellSize = 1f, Color? gizmosColor = null)
    {
        Gizmos.color = gizmosColor ?? Color.green;
        columns = ClampMin(columns, 0);
        rows = ClampMin(rows, 0);
        for (int i = 0; i < columns; i++)
        {
            for (int j = 0; j < rows; j++)
            {
                GizmosDrawSquare(new Vector2(i * cellSize + cellSize / 2f, j * cellSize + cellSize / 2f) + bottomLeftPivotPosition, cellSize, Gizmos.color);
            }
        }
    }
